  ...
  return (
    <div className="w-full space-y-8 animate-fade-in pb-12 select-none">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-xl sm:text-2xl font-black text-white tracking-tight leading-tight">Vendas & Transações</h1>
          <p className="text-xs sm:text-sm text-zinc-400 mt-1">Histórico detalhado dos pagamentos efetuados pelos utilizadores.</p>
        </div>
        <div className="flex items-center gap-2 sm:gap-3 w-full sm:w-auto">
          <button 
            onClick={fetchTransactions}
            className="flex-1 sm:flex-none flex items-center justify-center gap-2 bg-[#121215] border border-zinc-800 hover:border-zinc-700 text-zinc-300 hover:text-white px-3 py-2.5 sm:py-2 rounded-xl text-xs font-semibold transition-all shadow-sm"
          >
            <RefreshCw className={`w-3.5 h-3.5 ${isLoading ? 'animate-spin' : ''}`} /> Atualizar
          </button>
          <button 
            onClick={() => alert("Relatório de vendas exportado em CSV!")}
            className="flex-1 sm:flex-none flex items-center justify-center gap-2 bg-red-600 hover:bg-red-700 text-white px-3 py-2.5 sm:py-2 rounded-xl text-xs font-bold transition-all shadow-[0_0_15px_rgba(220,38,38,0.2)]"
          >
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 sm:gap-5">
        <div className="col-span-2 sm:col-span-1 bg-[#121215]/90 border border-zinc-800/80 rounded-2xl p-4 sm:p-5 shadow-xl backdrop-blur-xl flex items-center justify-between min-w-0 gap-2">
          <div className="flex-1 min-w-0">
            <p className="text-[10px] sm:text-xs font-semibold text-zinc-400 uppercase tracking-wider mb-1 truncate">Receita Total de Vendas</p>
            <h3 className="text-xl sm:text-2xl font-black text-emerald-400 truncate">MT {totalRevenue.toLocaleString('pt-BR')}</h3>
          </div>
        ...
        <div className="bg-[#121215]/90 border border-zinc-800/80 rounded-2xl p-3.5 sm:p-5 shadow-xl backdrop-blur-xl flex flex-col items-start gap-3 relative overflow-hidden min-w-0">
          ...
          <div className="w-full">
            <p className="text-[10px] sm:text-xs font-bold text-zinc-400 uppercase tracking-wider mb-0.5 truncate">Compras</p>
            <h3 className="text-lg sm:text-2xl font-black text-white truncate">MT {totalPurchases.toLocaleString('pt-BR')}</h3>
          </div>
        </div>
        ...
        <div className="bg-[#121215]/90 border border-zinc-800/80 rounded-2xl p-3.5 sm:p-5 shadow-xl backdrop-blur-xl flex flex-col items-start gap-3 relative overflow-hidden min-w-0">
          ...
          <div className="w-full">
            <p className="text-[10px] sm:text-xs font-bold text-zinc-400 uppercase tracking-wider mb-0.5 truncate">Alugueres</p>
            <h3 className="text-lg sm:text-2xl font-black text-amber-400 truncate">MT {totalRentals.toLocaleString('pt-BR')}</h3>
          </div>
        </div>
      </div>
      ...
      <div className="bg-[#121215]/90 border border-zinc-800/80 rounded-2xl p-4 shadow-xl backdrop-blur-xl flex flex-col md:flex-row items-center justify-between gap-4">
        ...
        <div className="relative w-full md:w-80">
          ...
          <input 
            ...
            className="w-full bg-zinc-900/80 border border-zinc-800 text-white text-xs rounded-xl pl-10 pr-4 py-3 sm:py-2.5 focus:outline-none focus:border-red-500/50 transition-colors"
          />
        </div>

        {/* Seleção de Filtros */}
        <div className="grid grid-cols-2 sm:flex sm:flex-wrap items-center gap-3 w-full md:w-auto">
          <div className="flex flex-col sm:flex-row sm:items-center gap-1.5 sm:gap-2 text-xs text-zinc-400 font-medium">
            <span className="flex items-center gap-1 sm:hidden"><Filter className="w-3 h-3 text-zinc-500" /> Tipo</span>
            <span className="hidden sm:inline-flex items-center gap-1.5"><Filter className="w-3.5 h-3.5 text-zinc-500" /> Tipo:</span>
            <select
              value={filterType}
              onChange={(e) => setFilterType(e.target.value)}
              className="w-full sm:w-auto bg-zinc-900 border border-zinc-800 text-white rounded-xl px-2.5 py-2.5 sm:py-1.5 outline-none focus:border-red-500"
            >
              <option value="all">Todos</option>
              <option value="buy">Compra (VIP)</option>
              <option value="rent">Aluguer (24h)</option>
            </select>
          </div>

          <div className="flex flex-col sm:flex-row sm:items-center gap-1.5 sm:gap-2 text-xs text-zinc-400 font-medium">
            <span className="flex items-center gap-1 sm:hidden">Estado</span>
